...
class PrettyDateTime
{
    ...
    private static function prettyFormat($difference, $unit)
    {
        ...
        $prepend = ($difference < 0) ? Driver::t('In')." " : '';
        $append = ($difference > 0) ? ' '. Driver::t("ago") : '';
    ...
    public static function parse(\DateTime $dateTime, \DateTime $reference = null)
    {
        ...
        if ($reference->format('Y/m/d') == $date) {
            if (0 <= $difference && $absDiff < self::MINUTE) {
                return Driver::t('Moments ago');
            } elseif ($difference < 0 && $absDiff < self::MINUTE) {
                return Driver::t('Seconds from now');
            } elseif ($absDiff < self::HOUR) {
                return self::prettyFormat($difference / self::MINUTE, Driver::t('minute') );
            } else {
                return self::prettyFormat($difference / self::HOUR, Driver::t('hour') );
            }
        }
        ...
        if ($yesterday->format('Y/m/d') == $date) {
            return Driver::t('Yesterday');
        } else if ($tomorrow->format('Y/m/d') == $date) {
            return Driver::t('Tomorrow');
        } else if ($absDiff / self::DAY <= 7) {
            return self::prettyFormat($difference / self::DAY, Driver::t('day') );
        } else if ($absDiff / self::WEEK <= 5) {
            return self::prettyFormat($difference / self::WEEK, Driver::t('week') );
        } else if ($absDiff / self::MONTH < 12) {
            return self::prettyFormat($difference / self::MONTH, Driver::t('month') );
        }

        // Over a year ago
        return self::prettyFormat($difference / self::YEAR, Driver::t('year') );
    }
}